...
C
      ...
      DOUBLE PRECISION, DIMENSION(M,N) :: DIMG,DREF,DBG,DISA,WORK
...
C         CALL WRITEIMAGE('CORR.FITS',(/1,1,1/),(/P,P,1/),DCORR)
     ...
     &      X-1,DBLE(0),2)
        ...
        CALL GETSNR(M,N,DR,DSNR,WORK,0)
        CALL DECONVWNR(M,N,WORK,DIMG,DREF,DSNR)
        ISAFILE=TRIM(PREFIX)//'_ISA_DECONV_'//
     &    TRIM(ADJUSTL(NUMSTR))//'.FITS'
        CALL WRITEIMAGE(ISAFILE,(/1,1,1/),(/M,N,1/),DIMG)
        DRMS=SQRT(SUM((DIMG-DISA)*(DIMG-DISA))/DBLE(NPIXS))
