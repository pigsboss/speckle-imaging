...
C ******************************************************************************
      SUBROUTINE DECONVRL(NX,NY,DG,DH,NUMIT,DF)
      IMPLICIT NONE
      INCLUDE 'fftw3.f'
      INTEGER, INTENT(IN) :: NX,NY,NUMIT
      DOUBLE PRECISION, INTENT(IN) :: DG(NX,NY),DH(NX,NY)
      DOUBLE PRECISION, INTENT(OUT) :: DF(NX,NY)
      INTEGER :: STATUS,INFO,PLANF,PLANB,K,X,Y
      DOUBLE PRECISION :: DIH(NX,NY)
...
C  Initialize reversed PSF, h(-x,-y), and OTFs.
      ...
      ZH=DCMPLX(DH)
      ZIH=DCMPLX(DIH)
      CALL ZIFFTSHIFT(NX,NY,ZH)
      CALL ZIFFTSHIFT(NX,NY,ZIH)
C
      
      CALL DFFTW_DESTROY_PLAN(PLANF)
      CALL DFFTW_DESTROY_PLAN(PLANB)
      RETURN
      END SUBROUTINE DCONVRL
C ******************************************************************************
